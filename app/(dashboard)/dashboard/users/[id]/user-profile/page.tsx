...
import UserEvents from "@/components/dashboard/usersComponents/UserEvents";
      ...
      <div className="bg-white rounded-3xl shadow-lg p-6 h-full flex flex-col justify-between gap-6">
        <div className="flex flex-row justify-between items-start gap-4">
          ...
          <div className="flex items-center gap-4">
            {user && (
              <EditBtn page="users" id={user?.id} details="user-profile" />
            )}
            {role === "Admin" && (
              <div>
                {user && (
                  <DeleteBtn item="User" id={user.id} deleteFn={mutate} />
                )}
              </div>
            )}
          </div>
        ...
        <div className="pt-4 border-t text-sm text-gray-500 flex flex-col md:flex-row justify-between gap-2">
          ...
          <span>Created-By: {user?.createdBy ? user.createdBy : "—"}</span>

          <span>
            Updated: {user?.updatedAt ? formatDate(user?.updatedAt) : "—"}
          </span>

          <span>Updated-By: {user?.updatedBy ? user.updatedBy : "—"}</span>
