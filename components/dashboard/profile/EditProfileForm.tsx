...
//   }, [profileFile, user.imageUrl]);
  ...
  const onSubmit: SubmitHandler<EditProfileFormFields> = async (data) => {
    const toastId = showToast("loading", { message: "Updating profile information..." });
    ...
    mutate(payload, {
      onSuccess: () => {
        showToast("success", { message: "Profile updated successfully!", toastId });
        reset();
      },
      onError: (err: any) => {
        showToast("error", { message: err.message || "Failed to update profile", toastId });
      },
    });
  };
